...
#if BT_USE_OPENMP && BT_THREAD_SAFE

#include <omp.h>

#endif  // #if BT_USE_OPENMP && BT_THREADSAFE

#if BT_USE_PPL && BT_THREAD_SAFE
...
{
	std::atomic<int>* aDest = reinterpret_cast<std::atomic<int>*>(&_lock);
	int expected = 0;
...
bool SpinMutex::TryLock()
{
	volatile long* aDest = reinterpret_cast<long*>(&_lock);
...
unsigned int bt::GetCurrentThreadIndex()
{
	...
	if (sThreadIndex == kNullIndex)
	{
		...
		assert(sThreadIndex < BT_MAX_THREAD_COUNT);
	}
#if BT_DETECT_BAD_THREAD_INDEX
...
bool bt::IsMainThread()
{
	return GetCurrentThreadIndex() == 0;
}

void bt::ResetThreadIndexCounter()
{
...
ITaskScheduler::ITaskScheduler(const char* name)
{
	_name = name;
	_savedThreadCounter = 0;
	_isActive = false;
}
...
void ITaskScheduler::Activate()
{
	...
	if (!_isActive)
	{
		gThreadCounter.mCounter = _savedThreadCounter;  // restore saved thread counter
		_isActive = true;
	}
}
...
void ITaskScheduler::DeActivate()
{
	if (_isActive)
	{
		_savedThreadCounter = gThreadCounter.mCounter;  // save thread counter
		_isActive = false;
	}
}
...
void bt::SetTaskScheduler(ITaskScheduler* ts)
{
	int threadId = GetCurrentThreadIndex();  // make sure we call this on main thread at least once before any workers run
	if (threadId != 0)
	{
		assert(!"btSetTaskScheduler must be called from the main thread!");
		return;
	}
	if (gBtTaskScheduler)
	{
...
ITaskScheduler* bt::GetTaskScheduler()
{
	return gBtTaskScheduler;
}

void bt::ParallelFor(int iBegin, int iEnd, int grainSize, const IParallelForBody& body)
{
#if BT_THREAD_SAFE
...
float bt::ParallelSum(int iBegin, int iEnd, int grainSize, const IParallelSumBody& body)
{
#if BT_THREAD_SAFE
...
///
class btTaskSchedulerSequential : public ITaskScheduler
{
public:
	btTaskSchedulerSequential() : ITaskScheduler("Sequential") {}
	virtual int GetMaxThreadsCount() const BT_OVERRIDE { return 1; }
	virtual int GetThreadsCount() const BT_OVERRIDE { return 1; }
	virtual void SetThreadsCount(int threadCount) BT_OVERRIDE {}
	virtual void ParallelFor(int iBegin, int iEnd, int grainSize, const IParallelForBody& body) BT_OVERRIDE
	{
		//BT_PROFILE("parallelFor_sequential");
		body.ForLoop(iBegin, iEnd);
	}
	virtual float ParallelSum(int iBegin, int iEnd, int grainSize, const IParallelSumBody& body) BT_OVERRIDE
	{
...
///
//class btTaskSchedulerOpenMP : public ITaskScheduler
//{
//	int m_numThreads;
//
//public:
//	btTaskSchedulerOpenMP() : ITaskScheduler("OpenMP")
//	{
//		m_numThreads = 0;
//	}
//	virtual int GetMaxThreadsCount() const BT_OVERRIDE
//	{
//		return omp_get_max_threads();
//	}
//	virtual int GetThreadsCount() const BT_OVERRIDE
//	{
//		return m_numThreads;
//	}
//	virtual void SetThreadsCount(int numThreads) BT_OVERRIDE
//	{
...
ITaskScheduler* bt::GetSequentialTaskScheduler()
{
...
ITaskScheduler* bt::GetOpenMPTaskScheduler()
{
#if BT_USE_OPENMP && BT_THREAD_SAFE
...
ITaskScheduler* bt::GetTBBTaskScheduler()
{
#if BT_USE_TBB && BT_THREAD_SAFE
...
ITaskScheduler* bt::GetPPLTaskScheduler()
{
#if BT_USE_PPL && BT_THREAD_SAFE
