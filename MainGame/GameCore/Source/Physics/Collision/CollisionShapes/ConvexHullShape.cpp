...
//                          Implement
//////////////////////////////////////////////////////////////////////////////////
#pragma region ConvexHullShape
ConvexHullShape::ConvexHullShape(const float* points, int numPoints, int stride) : PolyhedralConvexAABBCachingShape()
{
	_shapeType = (int)BroadPhaseNativeTypes::Convex_Hull_Shape;
	_unScaledPoints.resize(numPoints);

	unsigned char* pointsAddress = (unsigned char*)points;

	for (int i = 0; i < numPoints; i++)
	{
		float* point = (float*)pointsAddress;
		_unScaledPoints[i] = Vector3(point[0], point[1], point[2]);
...
*						AddPoint
*************************************************************************//**
*  @fn        void ConvexHullShape::AddPoint(const gm::Vector3& point, bool reCalculateLocalAABB)
*  @brief     Add hull vextex point
*  @param[in] Vector3 point
*  @param[in] bool reCalculateLocalAABB
*  @return 　　void
*****************************************************************************/
void ConvexHullShape::AddPoint(const Vector3& point, bool reCalculateLocalAABB)
{
	_unScaledPoints.push_back(point);
...
void ConvexHullShape::OptimizeConvexHull()
{
	...
	conv.Compute(&_unScaledPoints[0], sizeof(Vector3), _unScaledPoints.size(), 0.f, 0.f);
	int numVerts = conv.Vertices.size();
	_unScaledPoints.resize(0);
	for (int i = 0; i < numVerts; i++)
	{
		_unScaledPoints.push_back(conv.Vertices[i]);
	}
}
/****************************************************************************
*						LocalGetSupportingVertexWithoutMargin
*************************************************************************//**
*  @fn        Vector3 ConvexHullShape::LocalGetSupportingVertexWithoutMargin(const Vector3& vector)
*  @brief     Get local supporting vertex
*  @param[in] Vector3 vector
*  @return 　　Vector3
*****************************************************************************/
Vector3 ConvexHullShape::LocalGetSupportingVertexWithoutMargin(const Vector3& vector) const
{
	Vector3 supVec(Scalar(0.), Scalar(0.), Scalar(0.));
	Scalar maxDot = Scalar(-FLT_MAX);

	// Here we take advantage of dot(a, b*c) = dot(a*b, c).  Note: This is true mathematically, but not numerically.
	if (0 < _unScaledPoints.size())
	{
		Vector3 scaled = vector * _localScaling;
		int index      = MaxDot(scaled, &_unScaledPoints[0], _unScaledPoints.size(), maxDot);  // FIXME: may violate encapsulation of m_unscaledPoints
		return _unScaledPoints[index] * _localScaling;
	}
...
*************************************************************************//**
*  @fn        Vector3 BCpnvexHullShape::LocalGetSupportingVertex(const Vector3& vector) const
*  @brief     Local get supporting vertex
*  @param[in] Vector3& vector
*  @return 　　Vector3
*****************************************************************************/
Vector3 ConvexHullShape::LocalGetSupportingVertex(const Vector3& vector) const
{
	...
	if (GetMargin() != 0.0f)
	{
		...
		{
			vecnorm = Vector3(Scalar(-1.), Scalar(-1.), Scalar(-1.));
		}
...
*  @return 　　void
*****************************************************************************/
void ConvexHullShape::BatchedUnitVectorGetSupportingVertexWithoutMargin(const gm::Vector3* vectors, gm::Vector3* supportVerticesOut, int numVectors) const
{
	Scalar newDot;

	for (int j = 0; j < numVectors; j++)
	{
		Vector3 vec = vectors[j] * _localScaling;  // dot(a*b,c) = dot(a,b*c)
		if (0 < _unScaledPoints.size())
		{
			int i = (int)MaxDot(vec, &_unScaledPoints[0], _unScaledPoints.size(), newDot);
...
*						Project
*************************************************************************//**
*  @fn        void ConvexHullShape::Project(const AffineTransform& transform, const Vector3& direction, Scalar& minProj, Scalar& maxProj, Vector3& witnesPtMin, Vector3& witnesPtMax) const
*  @brief     Project 
*  @param[in] Transform transform
*  @param[in] Vector3 direction
*  @param[out] Scalar& minProj, 
*  @param[out] Scalar& maxProj, 
*  @param[out] Vector3& witnesPtMin, 
*  @param[out] Vector3& witnesPtMax
*  @return 　　void
*****************************************************************************/
void ConvexHullShape::Project(const AffineTransform& transform, const Vector3& direction, Scalar& minProj, Scalar& maxProj, Vector3& witnesPtMin, Vector3& witnesPtMax) const
{
	minProj = FLT_MAX;
	maxProj = -FLT_MAX;

	int numVerts = _unScaledPoints.size();
	for (int i = 0; i < numVerts; i++)
	{
		Vector3 vtx = _unScaledPoints[i] * _localScaling;
		Vector3 pt  = transform * vtx;
...
int ConvexHullShape::GetVerticesCount() const
{
	return _unScaledPoints.size();
}
int  ConvexHullShape::GetEdgesCount() const
{
	return _unScaledPoints.size();
}

void ConvexHullShape::GetEdge(int index, gm::Vector3& pa, gm::Vector3& pb) const
{
	int index0 = index       % _unScaledPoints.size();
	int index1 = (index + 1) % _unScaledPoints.size();
	pa = GetScaledPoint(index0);
	pb = GetScaledPoint(index1);
}

void ConvexHullShape::GetVertex(int index, Vector3& vertex) const
{
	vertex = GetScaledPoint(index);
}

int  ConvexHullShape::GetPlanesCount() const
{
	return 0;
}

void ConvexHullShape::GetPlane(Vector3& planeNormal, Vector3& planeSupport, int i) const
{
	assert(0);
}

bool ConvexHullShape::IsInside(const Vector3& pt, float tolerance) const
{
	assert(0);
	return false;
}
...
*  @return 　　void
*****************************************************************************/
void ConvexHullShape::SetLocalScaling(const Vector3& scaling)
{
...
#pragma endregion ConvexHullShape

#pragma region HullShape
HullShape::HullShape(const ConvexShape* shape)
{
	_shape = shape;
	_vertices.clear();
	_indices .clear();
...
bool HullShape::BuildHull(float margin, int highres)
{
	int numSampleDirections = highres ? NUM_UNITSPHERE_POINTS_HIGHRES : NUM_UNITSPHERE_POINTS;
	Vector3 supportPoints[NUM_UNITSPHERE_POINTS_HIGHRES + MAX_PREFERRED_PENETRATION_DIRECTIONS * 2];
	int i;
	for (i = 0; i < numSampleDirections; i++)
	{
		supportPoints[i] = _shape->LocalGetSupportingVertex(GetUnitSpherePoints(highres)[i]);
	}

	int numPDA = _shape->GetPreferredPenetrationDirectionsCount();
	if (numPDA)
	{
		for (int s = 0; s < numPDA; s++)
		{
			Vector3 norm;
			_shape->GetPreferredPenetrationDirection(s, norm);
			supportPoints[i++] = _shape->LocalGetSupportingVertex(norm);
			numSampleDirections++;
		}
	}
	HullDesc hd;
	hd.Flags = QF_TRIANGLES;
	hd.Vcount = static_cast<unsigned int>(numSampleDirections);

#ifdef BT_USE_DOUBLE_PRECISION
	hd.mVertices = &supportPoints[0];
	hd.mVertexStride = sizeof(VVector3);
#else
	hd.Vertices     = &supportPoints[0];
	hd.VertexStride = sizeof(Vector3);
#endif
	...
	_vertices.resize(static_cast<int>(hr.NumOutputVertices));

	for (i = 0; i < static_cast<int>(hr.NumOutputVertices); i++)
	{
		_vertices[i] = hr.OutputVertices[i];
	}
	_indexCount = hr.NumIndices;
	_indices.resize(static_cast<int>(_indexCount));
	for (i = 0; i < static_cast<int>(_indexCount); i++)
	{
		_indices[i] = hr.m_Indices[i];
	}
...
int HullShape::TriangleCount() const
{
	return static_cast<int>(_indexCount / 3);
}

int HullShape::VerticesCount() const
{
	return _vertices.size();
}
int HullShape::IndicesCount() const
{
	return static_cast<int>(_indexCount);
}

/****************************************************************************
*                      GetUnitSpherePoints
*************************************************************************//**
*  @fn        gm::Vector3* HullShape::GetUnitSpherePoints(int highres)
*  @brief     Get unit sphere mesh
*  @param[in] int highres
*  @param[out] gm::Vector3*
*  @return 　　void
*****************************************************************************/
gm::Vector3* HullShape::GetUnitSpherePoints(int highres)
{
